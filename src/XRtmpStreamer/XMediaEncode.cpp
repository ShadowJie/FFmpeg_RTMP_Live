...
public:
	void Close()
	{
		...
		if (asc)
		{
			swr_free(&asc);//会赋值NULL
		}
		if (yuv)
		{
			av_frame_free(&yuv);//会内部赋值NULL
		}
		if (vc)
		{
			avcodec_free_context(&vc);//会赋值NULL
		}

		if (pcm)
		{
			av_frame_free(&pcm);//会内部赋值NULL
		}
	...
	bool InitAudioCodec()
	{
		...
		ac->sample_fmt = AV_SAMPLE_FMT_FLTP;//AV_SAMPLE_FMT_FLTP
	...
	bool InitVideoCodec()
	{
		...
		vc->width = outWidth;
		vc->height = outHeight;
		//vc->time_base = { 1,fps };//	1/fps
		vc->framerate = { fps,1 };//	fps/1

		//画面组的大小，多少帧一个关键帧
		vc->gop_size = 50;
		vc->max_b_frames = 0;// b帧
	...
	XData RGBToYUV(XData d)
	{
		...
		//bgrbgrbgr	交错
		//plane indata[0] bbb  indata[1] ggg  indata[2] rrr   
